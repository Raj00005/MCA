...
(function ($, window, document, undefined) {
  ...
  var Owl = function (element, options) {
    ...
    plugin.onResize = function () {
      if (plugin.isVisible() && plugin._invalidated.width) {
        plugin.enter('resizing');
        plugin.trigger('resize').isDefaultPrevented() ? plugin.leave('resizing') : (plugin._invalidated.width = false, plugin.refresh());
    ...
    plugin.enter = function (state) {
      if (plugin._states.current[state] === undefined) {
        plugin._states.current[state] = 0;
      }

      plugin._states.current[state]++;
    };

    plugin.leave = function (state) {
      plugin._states.current[state]--;
    };
